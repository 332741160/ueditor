...
define(function (require) {
    ...
    var htmlparser = require('../core/htmlparser');
    ...
    return function () {
        ...
        me.ready(function () {
            ...
            for (var p in customStyle) {
                if (p === 'dash' || p === 'dot') {
                    customCss.push('li.list-' + customStyle[p]
                        + '{background-image:url(' + liiconpath + customStyle[p] + '.gif)}');
                    customCss.push('ul.custom_' + p
                    ...
                    customCss.push('ol.custom_' + p
                        + '{list-style:none;}ol.custom_' + p
                        + ' li{background-position:0 3px;background-repeat:no-repeat}');
                }
                switch (p) {
                    case 'cn':
            ...
            utils.cssRule('list', 'ol,ul{margin:0;pading:0;'
                + (browser.ie ? '' : 'width:95%') + '}li{clear:both;}' + customCss.join('\n'), me.document);
        });
        // 单独处理剪切的问题
        me.ready(function () {
        ...
        me.addListener('beforepaste', function (type, html) {
            ...
            var rng = me.selection.getRange();
            var li;
            var root = htmlparser(html.html, true);
            if (li = domUtils.findParentByTagName(rng.startContainer, 'li', true)) {
        ...
        me.addInputRule(function (root) {
            utils.each(root.getNodesByTagName('li'), function (li) {
                var tmpP = uNode.createElement('p');
            ...
            if (me.options.autoTransWordToList) {
                ...
                utils.each(root.getNodesByTagName('p'), function (node) {
                    ...
                    if (node.parentNode.tagName !== 'li' && (type = checkListType(node.innerText(), node))) {
                        ...
                        var list = uNode.createElement(me.options.insertorderedlist.hasOwnProperty(type) ? 'ol' : 'ul');
                        if (customStyle[type]) {
        ...
        function adjustListStyle(doc, ignore) {
            utils.each(domUtils.getElementsByTagName(doc, 'ol ul'), function (node) {
                ...
                !ignore && adjustList(node, node.tagName.toLowerCase(), getStyle(node)
                    || domUtils.getStyle(node, 'list-style-type'), true);
            });
        }
        function adjustList(list, tag, style, ignoreEmpty) {
            var nextList = list.nextSibling;
            if (nextList && nextList.nodeType === 1
        ...
        me.addListener('keydown', function (type, evt) {
            ...
            if (keyCode === 13 && !evt.shiftKey) { // 回车
                var rng = me.selection.getRange();
                var parent = domUtils.findParent(rng.startContainer, function (node) {
                ...
                if (start && end && start === end) {
                    ...
                    if (!range.collapsed) {
                        ...
                        if (start && end && start === end) {
                            ...
                            li = domUtils.findParentByTagName(tmpRange.startContainer, 'li', true);
                ...
                if (range.collapsed && domUtils.isStartInblock(range)) {
                    ...
                    if (li && domUtils.isStartInblock(tmpRange)) {
                        ...
                        if (start && start !== li.firstChild) {
                            parentList = domUtils.findParentByTagName(start, ['ol', 'ul']);
            ...
            if (keyCode === 8) {
                ...
                if (list = domUtils.findParentByTagName(rng.startContainer, ['ol', 'ul'], true)) {
                    adjustList(list, list.tagName.toLowerCase(),
                        getStyle(list) || domUtils.getComputedStyle(list, 'list-style-type'), true);
                }
            }
        ...
        me.addListener('tabkeydown', function () {
            ...
            if (li) {
                ...
                if (range.collapsed) {
                    ...
                    var index = utils.indexOf(listStyle[list.tagName], getStyle(parentLi)
            ...
            execCommand: function (command, style) {
                ...
                while (current && current !== bk.end
                    ...
                    if (current.nodeType === 3 || dtd.li[current.tagName]) {
                        ...
                        if (domUtils.isEmptyNode(li)) {
                            tmpNode = range.document.createElement('p');
                            while (li.firstChild) {
                ...
                // 去掉冗余的tmpDiv
                var tmpDivs;
                for (i = 0, ci, tmpDivs = domUtils.getElementsByTagName(list, 'div'); ci = tmpDivs[i++];) {
