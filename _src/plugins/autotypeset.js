...
define(function (require) {
    ...
    return function () {
        ...
        function autotype(type, html) {
            ...
            for (var i = 0, ci; ci = nodes[i++];) {
                ...
                if (opt.removeEmptyNode) {
                    if (opt.removeTagNames[ci.tagName.toLowerCase()]
                        && domUtils.hasNoAttributes(ci) && domUtils.isEmptyBlock(ci)) {
                        domUtils.remove(ci);
                    }
                }
            ...
            if (opt.tobdc) {
                var root = htmlparser(cont.innerHTML);
                root.traversal(function (node) {
                    if (node.type === 'text') {
                        node.data = ToDBC(node.data);   // eslint-disable-line
                    }
            ...
            if (opt.bdc2sb) {
                root = htmlparser(cont.innerHTML);
                root.traversal(function (node) {
                    if (node.type === 'text') {
                        node.data = DBC2SB(node.data);    // eslint-disable-line
                    }
