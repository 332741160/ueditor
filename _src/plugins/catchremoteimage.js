    ...
    return function () {
        ...
        me.addListener('catchRemoteImage', function () {
                    ...
                    success: function (r) {
                        try {
                            var info = r.state !== undefined
                                ? r
                                : eval('(' + r.responseText + ')');   // eslint-disable-line
                        }
                        catch (e) {
                            return;
                        }
                            ...
                            for (j = 0; cj = list[j++];) {
                                if (oldSrc === cj.source && cj.state === 'SUCCESS') { // 抓取失败时不做替换处理
                                    ...
                                    domUtils.setAttributes(ci, {
                                        src: newSrc,
                                        _src: newSrc    // eslint-disable-line
                                    });
                                    break;
                                }
            ...
            function catchremoteimage(imgs, callbacks) {
                ...
                var url = utils.formatUrl(catcherActionUrl + (catcherActionUrl.indexOf('?') === -1 ? '?' : '&') + params);    // eslint-disable-line
