...
define(function (require) {
    ...
    return function () {
        ...
        function mergesibling(rng, cmdName, value) {
            ...
            utils.each(domUtils.getElementsByTagName(common, 'span'), function (span) {
                ...
                if (/border/i.test(span.style.cssText)
                    && span.parentNode.tagName === 'SPAN' && /border/i.test(span.parentNode.style.cssText)) {
        ...
        me.addInputRule(function (root) {
            utils.each(root.getNodesByTagName('u s del font strike'), function (node) {
                if (node.tagName === 'font') {
                    var cssStyle = [];
                    for (var p in node.attrs) {   // eslint-disable-line
                        switch (p) {
                            case 'size':
        ...
        //        });
        //    });
        for (var p in fonts) {        // eslint-disable-line
            (function (cmd, style) {  // eslint-disable-line
                UE.commands[cmd] = {
                    execCommand: function (cmdName, value) {    // eslint-disable-line
                        value = value || (this.queryCommandState(cmdName)
                            ? 'none' : cmdName === 'underline'
                            ? 'underline' : cmdName === 'fontborder'
